...
const SHARE_OPTIONS = [
  ...
  },
  {
    ...
    getUrl: (url: string) => `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent('Check out this post')}`,
  },
  {
    name: 'Facebook',
    color: '#1877F2',
  ...
  },
  {
    ...
    getUrl: (url: string) => `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent('Check out this post')}`,
  },
  {
    name: 'Email',
    color: '#EA4335',
    ...
    getUrl: (url: string) => `mailto:?subject=${encodeURIComponent('Check out this post')}&body=${encodeURIComponent(url)}`,
    isEmail: true,
  },
]
...
const ShareDialog: React.FC<ShareDialogProps> = ({ postId, onClose }) => {
  ...
  const handleShareOption = useCallback(
    (option: (typeof SHARE_OPTIONS)[number]) => {
      const url = option.getUrl(shareUrl)
      if (option.isEmail) {
