 ...
 *
 * Layout: responsive Match Control hero/auth grid followed by pricing and legal links
 */
...
import { getAuthLandingView } from "@/features/auth/utils/authNavigation";

export default function LandingPage() {
  const location = useLocation();
  const initialAuthView = getAuthLandingView(location.search);
  const [signupRequestKey, setSignupRequestKey] = useState(0);
  const requestedAuthView = signupRequestKey > 0 ? 'signup' : initialAuthView;
  ...
  return (
    <main className="relative isolate min-h-dvh w-full overflow-hidden bg-(--mc-color-canvas) text-(--mc-color-text)">
      ...
      <div className="mx-auto grid w-full max-w-[82rem] items-center gap-8 px-4 pb-12 pt-8 sm:px-6 sm:pt-12 lg:grid-cols-[minmax(0,1.08fr)_minmax(24rem,0.72fr)] lg:gap-14 lg:px-10 lg:pb-20 lg:pt-20">
        <HeroSection />
        <AuthSection
          key={`${requestedAuthView}-${signupRequestKey}`}
          initialView={requestedAuthView}
        />
      </div>

      <PricingSection onRequestSignup={() => setSignupRequestKey((key) => key + 1)} />
      <FooterSection />
    </main>
  );
}
