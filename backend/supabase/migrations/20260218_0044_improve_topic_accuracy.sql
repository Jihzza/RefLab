-- Migration: Improve accuracy by topic to use test_questions.topic
-- Created: 2026-02-18
-- Description: Updates dashboard RPC to calculate topic accuracy from test_questions.topic
--              instead of tests.topic, allowing random tests to contribute to topic stats
...
BEGIN
  ...
  -- 2. Accuracy by Topic (using test_questions.topic instead of tests.topic)
  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json)
  INTO v_accuracy_by_topic
  FROM (
    SELECT
      COALESCE(tq.topic, 'Uncategorized') AS topic,
      round(
